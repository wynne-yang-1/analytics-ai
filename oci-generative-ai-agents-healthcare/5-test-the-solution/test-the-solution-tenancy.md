...
- Uploaded the clinical policy manuals to a storage bucket.
...
1. Once the agent is showing as **Active**, click the **healthcare authorization agent** in the **Agents** list.
...
1. On the top of the page, the **Agent** drop down should show **healthcare authorization agent** and the **Agent endpoint** drop down should show the newly created endpoint.
...
1. Our next question would be: _Which healthcare specialist has the most authorization requests assigned?_ Let's see if the agent will be able to figure that out...
1. Click the **Submit** button.

    ![Screenshot showing the second question for the agent](./images/send-second-question.png)

1. The agent shows the correct answer: **Olivia Brown**. Using the magic of Large Language Models (LLMs) and the clues we've left in the configuration of the agent and tools, the agent was able to decipher that the healthcare specialist with the most authorization requests assigned to them.
...
1. Next we'll ask the following: _List authorization requests that have been in progress for more than 7 days._
1. Click the **Submit** button.

    ![Screenshot showing the third question for the agent](./images/send-third-question.png)

1. The agent returned information on the one authorization request that has been in progress for more than 7 days.

    ![Screenshot showing the third question for the agent](./images/third-question-response.png)

1. Now that we have information about the requests, let's see if we can pull up a clinical policy document which can help us define MRI authorization criteria. Type the following question: _Retrieve the policy document section that defines MRI authorization criteria and clinical requirements._
1. Click the **Submit** button.

    ![Screenshot showing the fourth question response](./images/send-fourth-question.png)
1. As you can see, for this question, the agent figured out that the information required might be in the clinical policy manuals. For this task it employed the RAG tool which searched for the relevant information in our clinical policy docs stored in object storage. Feel free to look at the traces for this interaction which show the steps the agent took to give us the information we needed. In the response you can see that a summary of the document was provided, but, also, if you expand the **View citations** section, you'll be able to see a reference to the document(s) which were used to compose the reply with a direct link to the file(s), the page(s) from which content was extracted and more.

    ![Screenshot showing the fourth question response](./images/fourth-question-response.png)

1. Next we'll ask the following: _Identify any approved authorization requests that violate clinical policy (diagnosis codes or urgency levels); cite the rule and the record._
1. Click the **Submit** button.

    ![Screenshot showing the fifth question for the agent](./images/send-fifth-question.png)

1. The agent successfully detected an approved authorization request whose clinical criteria was inconsistent with the requirements outlined in the clinical policy document.

    ![Screenshot showing the fifth question response](./images/fifth-question-response.png)
...
- _What are the minimum clinical requirements for MRI vs CT Scan imaging?_
- _Show the distribution of urgency levels by service type_
- _Provide the policy language for specialty medication authorization and list the denied specialty medication requests from the database_
...
Although our use-case was focused on healthcare authorization, the OCI AI Agents service can be used to fuel many different use-cases which require deep understanding and retrieval of information from internal data sources, reasoning over the data, summarizing it, providing insights and more.
